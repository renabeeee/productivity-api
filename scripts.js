...
function startTask(index) {
  if (currentTaskIndex >= 0) {
    ...
    document.getElementById(
      `pause-resume-button-${currentTaskIndex}`
    ).style.display = "none";
  }
  ...
  document.getElementById(
    "timer-display"
  ).innerText = `${currentTask.name}: ${currentTask.duration} minutes`;
  ...
  document.getElementById(`pause-resume-button-${index}`).style.display =
    "inline-block";
}
...
function notifyNextTask() {
  ...
  showNotification(
    `Time's up! Your next task is ${tasks[currentTaskIndex].name}.`
  );
  displayTasks();
